...
- 语音存在不等于真人对话感；播放控制、响应节奏、状态提示和连续对话都会影响体验。
...
|---|---|---|---|---|---|---|---|
| F-001 | T001 | 希望考官主要以语音交互，并有明确播放或重播按钮。 | 语音播放入口是否足够可发现，是否支持用户主动控制。 | 已有 TTS、移动端 Play 提示和 pending speech 播放卡片。 | 暂不重构语音系统，先验证播放/重播入口是否容易理解。 | P1 | 待验证 |
...
- `v2/backend/routes/audio.py` 提供 `/api/tts`。
- `v2/backend/audio_services.py` 使用 gTTS 并有 TTS cache。
- `v2/frontend/src/hooks/useSpeechPlayback.js` 管理播放、浏览器自动播放限制和 pending speech。
- `v2/frontend/src/components/layout/PendingSpeechCard.jsx` 提供 `Play Victoria` 按钮。
- `v2/frontend/src/components/layout/MobileToasts.jsx` 在移动端显示 `Victoria's voice is ready` 和 `Play`。

产品判断：当前更可能是“可发现性和控制感”问题，而不是完全没有语音播放功能。用户可能不知道什么时候可以播放、是否能重播、播放失败时该怎么做。

产品决策：暂不重构整个语音系统。下一步先通过 3 名测试者验证：
